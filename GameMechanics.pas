...
IMPLEMENTATION
  procedure Enemy.Create(x, y, MovePointL, MovePointR: integer; fname: string);
  begin
    fx := x;
    fy := y;
  ...
  procedure Enemy.setX(x: integer);
  begin
    fp.MoveTo(x, fy);
    fx := x;
  end;
  
  function Enemy.getX: integer;
  begin
    Result := fx;
  end;
    
    function Enemy.getDirection: shortint;
    begin
      Result := fdirection;
    end;
    
    procedure Enemy.setDirection(value: shortint);
    begin
      fdirection := value;
    end;
    
    function Enemy.getMovePointL: integer;
    begin
      Result := fMovePointL;
    end;
    
    function Enemy.getMovePointR: integer;
    begin
      Result := fMovePointR;
    end;
    
    function Enemy.getCollider: PictureABC;
    begin
      Result := fp;
    end;
...
begin
  ...
  begin
    ...
    if Player.Intersect(Level1Enemy.collider)
    then
      begin
        if velocityY = 0
        then
